...
function buildOrderCard(item) {
  ...
  card.innerHTML = `
        <div class="img-wrap"><img src="${item.image}" alt="${escapeHtml(
    item.name
  )}"></div>
        <div class="order-main">
          ...
          <div class="controls">
            <div class="qty-box">
              <button title="Kurangi" onclick="changeDurasi('${
                item.id
              }', -1)">−</button>
              <div style="min-width:8px"></div>
              <div style="font-weight:600"> <span id="durasi-${item.id}">${
    item.durasi
  }</span> Hari </div>
              <button title="Tambah" onclick="changeDurasi('${
                item.id
              }', 1)">+</button>
              ...
              <div style="min-width:120px;text-align:center">
                ...
                <div style="font-weight:800" id="qty-${item.id}">${
    item.qty
  }</div>
              </div>
            </div>
          ...
          <div style="margin-top:10px;color:var(--muted);font-size:13px" id="dates-${
            item.id
          }">
        ...
        <div class="order-side">
          <div style="text-align:right">
            ...
            <div class="price-big">Rp. <span id="total-${item.id}">${money(
    calcTotal(item)
  )}</span></div>
          </div>
          <div>
            <button class="btn-yellow" onclick="openModal('${
              item.id
            }')">Pesan</button>
          </div>
        </div>
...
tabs.forEach((t) => {
  t.addEventListener("click", () => {
    ...
    document.getElementById("tab-resi").style.display =
      which === "resi" ? "block" : "none";
    document.getElementById("tab-selesai").style.display =
      which === "selesai" ? "block" : "none";
  });
});
...
async function startPayment() {
  ...
  // Tampilkan loading (bisa pakai spinner yang sudah ada)
  const wait = document.getElementById("waitingCard");
  wait.classList.add("show");
  document.getElementById("waitBar").style.width = "50%"; // Indikasi loading

  // Siapkan data untuk dikirim ke Backend
  const payload = {
    customer_name: "Pelanggan Demo", // Bisa diambil dari input form jika ada
    customer_phone: "08129999999", // Bisa diambil dari input form jika ada
    items: [
      {
        id: currentModalProduct.id,
        name: currentModalProduct.name,
        price: currentModalProduct.pricePerDay,
        qty: currentModalProduct.qty,
        durasi: currentModalProduct.durasi,
      },
    ],
    ...
    if (result.success) {
      ...
      window.snap.pay(result.token, {
        onSuccess: function (result) {
          // Pembayaran Berhasil
          console.log(result);
          showSuccess(); // Panggil fungsi sukses bawaan Anda
        },
